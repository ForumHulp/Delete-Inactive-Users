...
class delete_inactive_users extends \phpbb\cron\task\base
{
	...
	*/
	public function run()
	{
		...
		$sql = 'SELECT user_id, username, user_regdate FROM ' . USERS_TABLE . ' WHERE user_type = ' . USER_INACTIVE . ' AND user_inactive_reason = 1 AND user_inactive_time <> 0 AND user_new = 1 AND user_regdate < ' . $expire_date;
